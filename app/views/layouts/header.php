...
$navigationConfig = [
    ...
    [
        ...
        'children' => [ [ 'label' => 'Requests', 'url' => 'It/requests', ], ]
    ],
    [
        'label' => 'Rap',
        ...
        'children' => [ [ 'label' => 'Calendar Of Activities', 'url' => 'Rap/calendar', ], ]
    ],
    [
        'label' => 'SES',
        ...
        'children' => [ [ 'label' => 'SES Data', 'url' => 'Ses/data', ], ]
    ],
];
...
function renderNavigationItems($items, $currentPath, $basePathStripped, $isDropdown = false) {
    ...
    foreach ($items as $item) {
        if (isset($item['capability']) && !userHasCapability($item['capability'])) { continue; }
        $itemVisible = true; 
        if (isset($item['children'])) {
            $childHtml = renderNavigationItems($item['children'], $currentPath, $basePathStripped, true);
            if (empty(trim($childHtml))) { 
                if (!isset($item['capability'])) { $itemVisible = false; }
            }
            if ($itemVisible) $hasVisibleChild = true; 
        } else {
             if (isset($item['capability']) && !userHasCapability($item['capability'])) { $itemVisible = false; }
            if ($itemVisible) $hasVisibleChild = true;
        }
        ...
        if (isset($item['type']) && $item['type'] === 'divider') {
            ...
            if (isset($item['icon']) && !$isDropdown) { $html .= '<i class="' . htmlspecialchars($item['icon']) . '"></i>'; }
            $html .= htmlspecialchars($item['label']);
            $html .= '</a></li>';
            if (!$isDropdown) $hasVisibleChild = true;
        }
...
<head>
    ...
    <style>
        ...
        .main-content { min-height: calc(100vh - 70px - 70px - 3rem); }
        ...
        /* Optional: Adjust DataTables styling if needed */
